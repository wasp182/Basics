...
# use this dictionary for dictwriter()

def dictkey(d:dict) -> str:
    return d['country']


column = ['country','gold','silver','bronze']
file_name= "country.csv"
with open(file_name,'w',encoding="utf-8",newline="") as opstream:
    writer = csv.DictWriter(opstream,fieldnames=column,extrasaction='ignore')
    writer.writeheader()
    #1 writing row by row
    # for rows in medals_table:
    #     writer.writerow(rows)
    #2 writing it in one chunk
    # writer.writerows(medals_table)
    #3 writing in sorted foramt but dictioanry doesnt support sort function
    # key passed here doesnt include the function call as sorted will call this
    # fucntion if and when it needs it, we need fcintoin name only
    writer.writerows(sorted(medals_table, key=dictkey))
