...
for x in locations:
    dicts = exits.get(x)
    if 5 in dicts.values():
        list_location.append([x,dicts])

print(list_location)

# c1: comprehension version
loc = 5
list_location2 = [(x , locations[x], exits.get(x)) for x in locations if loc in exits[x].values() ]
print(list_location2)

# c2 : comprehension version
for loc in sorted(locations):
    list_location3 = [(x , locations[x], exits.get(x)) for x in locations if loc in exits[x].values() ]
    print("for location {} , we have these nearest exits: {}".format(loc,list_location3))
...
for loc in sorted(locations):
    ...
    for x in exits:
        if loc in exits[x].values():
            list_location4.append([x, locations[x], exits[loc].get(x)])
    print("for location {} , we have these nearest exits: {}".format(loc,list_location4))
